...
Start=cpuTimer();
  ...
  Start=cpuTimer();
    ...
    {
        CHECK(cudaSetDevice(i));
        CHECK(cudaMemcpyAsync(d_MatA[i],MatA+i*(Arows/ngpus)*Acols,                    
                 (Arows/ngpus)*Acols*sizeof(float),
                 cudaMemcpyHostToDevice,stream[i]));
        CHECK(cudaMemcpyAsync(d_MatB[i],MatB, 
                  Acols*Bcols*sizeof(float),cudaMemcpyHostToDevice,stream[i]));
        dim3 block(threads_x,threads_y,1);
        dim3 grid((Bcols+block.x-1)/block.x, (Arows/ngpus+block.y-1)/block.y, 1);
        MatMultOnGPU<<<grid, block,0,stream[i]>>>(d_MatA[i], d_MatB[i], d_MatC[i], 
                Arows/ngpus, Acols, Bcols);

        CHECK(cudaMemcpyAsync(gpu_MatC+i*(Arows/ngpus)*Bcols, d_MatC[i], 
    ...
    CHECK(cudaFreeHost(MatA)),  CHECK(cudaFreeHost(MatB)),  CHECK(cudaFreeHost(MatC)),      
    CHECK(cudaFreeHost(gpu_MatC));
    for(int i=0;i<ngpus;i++)
